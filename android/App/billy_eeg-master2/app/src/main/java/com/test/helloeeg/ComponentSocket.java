...
class ComponentSocket extends Thread {
    ...
    ComponentSocket(String addr, int port, Handler callbacks) {
        serverAddress = addr;
        serverPort = port;
        Log.d(HelloEEGActivity.TAG, "Server Address: " + serverAddress);
        Log.d(HelloEEGActivity.TAG, "Server Port: " + serverPort);
        callback = callbacks;
    }
    ...
    public void run() {
        ...
        while(!killThread){
            try {
                ...
                Message message = callback.obtainMessage(HelloEEGActivity.CONNECTED);
                ...
                while(true){
                    ...
                    uploader(kvList);
                    if (kvList.size() > 1) {
                        Log.e(HelloEEGActivity.TAG, "Received raw: <" + kvList.encodedString() + ">");
                        //Tell the activity that a new message has been received.
                        Message msg = callback.obtainMessage(HelloEEGActivity.MESSAGE_RECEIVED);
                ...
                e.printStackTrace();
                Message message = callback.obtainMessage(HelloEEGActivity.DISCONNECTED);
                callback.sendMessage(message);
            }
            try {
                Thread.sleep(100);
    ...
    void killThread() {
        ...
        Message message = callback.obtainMessage(HelloEEGActivity.DISCONNECTED);
        callback.sendMessage(message);
        Log.e(HelloEEGActivity.TAG, "Sock thread killed." );
    }
    //The function is called by the activity and used to set the output message
    void setMessage(KeyValueList kvList) throws Exception {
        if(msgEncoder!=null){
            try {
                Log.e(HelloEEGActivity.TAG, "Sending messsage." );
                uploader(kvList);
    ...
    void uploader(KeyValueList kvList) throws Exception {
    ...
    void ack(){
        ...
        try {
            ...
            Log.e(HelloEEGActivity.TAG, "IOException: " + e.toString());
        }
    }
}