...
public class CreateFlowerManagerGUI extends Application {
	...
	public static final String SCOPE = "SIS.Tien";
	// name of this component
    public static final String NAME = "FlowerManagerGUI";
...
class MonitorTask implements Runnable {
	...
	public void runTask() {
		while (running) {
			try {
				...
				conn.putPair("Scope", CreateFlowerManagerGUI.SCOPE);
				...
				conn.putPair("Name", CreateFlowerManagerGUI.NAME);
	...
	private void ProcessMsg(KeyValueList kvList) throws Exception {
		...
		if (!CreateFlowerManagerGUI.SCOPE.startsWith(scope)) {
			return;
		}
		...
		case "Connect":
			...
			if (!name.equals(CreateFlowerManagerGUI.NAME)&&!role.equals("Monitor")) {
		...
		case "Confirm":
			...
			break;
		case "Setting":
            if (receiver.equals(CreateFlowerManagerGUI.NAME))
            {
				List<String> receivers = new ArrayList<String>(Arrays.asList(new String[] { "FlowerPower" }));
                ...
                case "Activate":
                    //send activate message to all its sub components
					KeyValueList act = new KeyValueList();
					act.putPair("Scope", CreateFlowerManagerGUI.SCOPE);
					act.putPair("MessageType", "Setting");
					act.putPair("Sender", "FlowerGUI");
					act.putPair("Purpose", "Activate");
					
					for (String rec : receivers) {
						act.removePair("Receiver");
						act.putPair("Receiver", rec);
						encoder.sendMsg(act);
					}
                ...
                case "Kill":
                    //send kill message to all its sub components
					KeyValueList kill = new KeyValueList();
					kill.putPair("Scope", CreateFlowerManagerGUI.SCOPE);
					kill.putPair("MessageType", "Setting");
					kill.putPair("Sender", "FlowerGUI");
					kill.putPair("Purpose", "Kill");

					for (String rec : receivers) {
						kill.removePair("Receiver");
						kill.putPair("Receiver", rec);
                ...
                case "Deactivate":
                    //send deactivate message to all its sub components
					KeyValueList deact = new KeyValueList();
					deact.putPair("Scope", CreateFlowerManagerGUI.SCOPE);
					deact.putPair("MessageType", "Setting");
					deact.putPair("Sender", "FlowerGUI");
					deact.putPair("Purpose", "Deactivate");

					for (String rec : receivers) {
						deact.removePair("Receiver");
						deact.putPair("Receiver", rec);
						encoder.sendMsg(deact);
					}
