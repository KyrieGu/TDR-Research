...
public class CreateChiGUI extends Application {
	...
	public static final String SCOPE = "SIS.Chi";
	// name of this component
    public static final String NAME = "ChiGUI";
...
class MonitorTask implements Runnable {
	...
	public void runTask() {
		while (running) {
			try {
				...
				conn.putPair("Scope", CreateChiGUI.SCOPE);
				conn.putPair("MessageType", "Connect");
				conn.putPair("Role", "Monitor");
				conn.putPair("Name", CreateChiGUI.NAME);
	...
	private void ProcessMsg(KeyValueList kvList) throws Exception {
		...
		if (!CreateChiGUI.SCOPE.startsWith(scope)) {
			return;
		}
		...
		case "Connect":
			...
			if (!name.equals(CreateChiGUI.NAME)&&!role.equals("Monitor")) {
