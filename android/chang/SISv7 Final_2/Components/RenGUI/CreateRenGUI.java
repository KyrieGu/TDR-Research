...
public class CreateRenGUI extends Application {
    ...
    public static final String NAME = "RenGUI";
...
class MonitorTask implements Runnable {
	...
	public void runTask() {
		while (running) {
			try {
				...
				conn.putPair("Scope", CreateRenGUI.SCOPE);
				conn.putPair("MessageType", "Connect");
				conn.putPair("Role", "Monitor");
				conn.putPair("Name", CreateRenGUI.NAME);
	...
	private void ProcessMsg(KeyValueList kvList) throws Exception {
		...
		if (!CreateRenGUI.SCOPE.startsWith(scope)) {
			return;
		}
		...
		case "Connect":
			...
			if (!name.equals(CreateRenGUI.NAME)&&!role.equals("Monitor")) {
