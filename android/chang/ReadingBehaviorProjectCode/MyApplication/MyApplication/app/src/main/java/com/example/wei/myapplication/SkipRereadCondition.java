...
public class SkipRereadCondition extends ActionBarActivity implements Camera.PreviewCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.skip_reread_condition);
        context = getApplicationContext();
        preview = (FrameLayout) findViewById(R.id.preview_skip_reread);
        fpFeatureSupported = FacialProcessing.isFeatureSupported(FacialProcessing.FEATURE_LIST.FEATURE_FACIAL_PROCESSING);
        btn_next=(Button)findViewById(R.id.next_btn_skip_reread_condition);
        btn_finish=(Button)findViewById(R.id.finish_btn_skip_reread_condition);
        ...
        mPreview = new CameraSurfacePreview(SkipRereadCondition.this, cameraObj, faceProc);
        ...
        mWebView = (WebView) findViewById(R.id.webview_skip_reread);
    ...
    private void doStuff() {
        ...
        int paras=4;
        if(readingOrder[progressNum]==1 && count==2){
            paras=5;
        }
        else if((readingOrder[progressNum]==3)||(readingOrder[progressNum]==1 && count==3)||(readingOrder[progressNum]==2 && count==3))
        {
            paras=3;
        }
        //skip
        int randNum1 =(int)(Math.random()*paras) + 1;
        int randNum2 =(int)(Math.random()*(paras-randNum1)) + 1;
        //reread
        int randNum3 =(int)(Math.random()*paras) + 1;
        int randNum4 =(int)(Math.random()*(paras-randNum3)) + 1;
        while(randNum1==randNum3 && randNum2==randNum4)
        {
            randNum3 =(int)(Math.random()*paras) + 1;
            randNum4 =(int)(Math.random()*(paras-randNum3)) + 1;
        }
        if(randNum1<=randNum3){
            String strJavascript1 = "grayOutAndHighlight("+randNum1+","+randNum2+","+randNum3+","+randNum4+");";
            mWebView.evaluateJavascript(strJavascript1, valueCallBackTemp);
        }
        else{
            String strJavascript1 = "highlightAndGrayOut("+randNum3+","+randNum4+","+randNum1+","+randNum2+");";
            mWebView.evaluateJavascript(strJavascript1, valueCallBackTemp);
        }

        Toast.makeText(this, "Please wait until this toast totally disappear to begin reading!", Toast.LENGTH_SHORT).show();
        gazeDataString.append("starting,starting" + "\n");
        layoutResult.append("SKIP, "+randNum1+", "+randNum2+", REREAD, "+randNum3+", "+randNum4);
...
//    }
    ...
    public void startCamera(int cameraIndex) {
        ...
        preview = (FrameLayout) findViewById(R.id.preview_skip_reread);
        preview.addView(mPreview,0);
        cameraObj.setPreviewCallback(SkipRereadCondition.this);
    }
...
//                setUI(gazePointValue);
    ...
    public void skipRereadNext(View view) {
        ...
        //-------------Face Feature Saving----------------
        String filename0 = "FaceFeature-article"+readingOrder[progressNum]+"_"+count+"_SkipRereadCondition"+System.currentTimeMillis()+".txt";
        //"GazeReadingData.txt";
        try {
            File dir=null;
        ...
        // Gaze Data Saving-------------------------
        String filename = "Gaze-article"+readingOrder[progressNum]+"_"+count+"_SkipRereadCondition"+System.currentTimeMillis()+".txt";
        //"GazeReadingData.txt";
        try {
            File dir=null;
            OutputStreamWriter outputstreamwriter;
        ...
        String filename2 = "Layout-article"+readingOrder[progressNum]+"_"+count+"_SkipRereadCondition"+System.currentTimeMillis()+".txt";;
        try {
            File dir2=null;
...
//        }
    ...
    public void skipRereadFinish(View view) {
        //String directory0 = context.getExternalFilesDir(null) + "/DataCollectionExperiment/USERID_"+userid+"/";
        String directory0;
        if(warmup==1){
            directory0 = context.getExternalFilesDir(null) + "/DataCollectionExperiment/WarmUp/USERID_"+userid+"/";
